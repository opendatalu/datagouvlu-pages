...
L'objectif principal de cette ouverture des ensembles de données est leur interopérabilité, c'est-à-dire la possibilité de les mélanger avec d'autres ensembles de données. La combinaison de différents ensembles de données permet de créer des produits et des services plus nombreux et de meilleure qualité.
...
En vrac : Les données doivent être disponibles sous la forme d'un ensemble complet. Une API web ou un service similaire peuvent également être très utiles, mais ils ne remplacent pas l'accès en masse.
...
La publication de données brutes en vrac devrait être la préoccupation principale de toutes les initiatives de données ouvertes, car la fourniture d'une API entraîne des coûts considérables. L'accès aux données en vrac garantit ce qui suit
